...
public class Add_Provider_UI extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Add_Provider_UI frame = new Add_Provider_UI();
	...
	public void initComponents() {
		...
		lbl_TenNhaCC.setBounds(10, 11, 110, 14);
		...
		textField_SDT.setBounds(116, 52, 136, 20);
	...
	private void initId() {
		List<Providers> list = provider_BUS.listProviders(); 
		int idInit = 1;
		for (Providers p : list) {
			int maSP = p.getIdProvider();
			if (idInit == maSP) {
				idInit++;
			}
		}
